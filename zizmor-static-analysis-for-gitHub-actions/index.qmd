...
A GitHub Actions workflow is an executable, security-sensitive program, not just
configuration. A compromised step may have access to secrets, repository
...
between these things — without needing to execute the workflow.
...
The standard way to use a GitHub Action is to reference the major version. Where
the major-version tag here currently identifies the latest `v7` release.
...
An alternative approach would be to reference the specific patch release. These
...
But `v7` may later be moved to a new patch release.

```{.yaml code-line-numbers="false"}
- uses: actions/checkout@v7.0.2
```
...
- They are not equivalent guarantees.
- `v7.0.1` is also a tag and therefore mutable.
- Only a full commit SHA identifies immutable Git content from the workflow’s
  perspective.
...
The exact code being executed is now explicit.

::: {.callout-important}

Need to check the SHA is not from a fork.
...
Immutable releases improve what publishers can guarantee; SHA pinning gives
consumers an explicit, reviewable dependency lock.
...
Immutable releases are being increasingly used throughout GitHub, for example
[astral-sh/setup-uv](https://github.com/astral-sh/setup-uv)
{{< fa brands github >}} no longer used major versions. The current version is
`v9.0.0`, not `v9`.
...
update them and verify that version annotations match their SHAs. This can
alternatively be done though tools like
...
## Rule 1: [unpinned-uses](https://docs.zizmor.sh/audits/#unpinned-uses) {footer=false}

> When a `uses:` clause is not pinned by branch, tag, or SHA reference, GitHub
> Actions will use the latest commit on the referenced repository's default
> branch (or, in the case of Docker actions, the `:latest` tag).
