...
- A release-specific tag may be protected by an immutable release.
...
Verify that the SHA belongs to the action's original repository, rather than to
a fork.
...
exactly what the **consumer** has reviewed and chosen to execute.
...
{{< fa brands github >}} has stopped publishing moving major-version tags. For
example, it publishes `v9.0.0`, but not a moving `v9` tag.
